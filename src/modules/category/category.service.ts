...
export class CategoryService {
    ...
    async save(body: SaveCategoryDto): Promise<ApiResponse> {
        ...
        if (body.parentId) {
            const parentCategory = await this.categoryRepository.getByField({ _id: new Types.ObjectId(body.parentId), isDeleted: false });
    ...
    async update(id: string, body: UpdateCategoryDto): Promise<ApiResponse> {
        ...
        const categoryToUpdate = await this.categoryRepository.getByField({ _id: new Types.ObjectId(id), isDeleted: false });
        ...
        if (body.parentId) {
            ...
            if (parentCategory._id.toString() === categoryToUpdate._id.toString()) throw new BadRequestException('A category cannot be its own parent!');
        }
    ...
    async delete(id: string): Promise<ApiResponse> {
        ...
        const categoryToDelete = await this.categoryRepository.getByField({ _id: new Types.ObjectId(id), isDeleted: false });
    ...
    async statusUpdate(id: string, body: StatusCategoryDto): Promise<ApiResponse> {
        ...
        const categoryToStatusChange = await this.categoryRepository.getByField({ _id: new Types.ObjectId(id), isDeleted: false });
